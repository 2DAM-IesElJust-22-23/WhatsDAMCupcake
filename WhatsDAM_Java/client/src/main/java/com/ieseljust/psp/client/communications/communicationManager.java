...
public class communicationManager {
    ...
    /* Aquesta classe s'encarrega de la gesti贸 de la
    ...
    public static void sendMessage(Message m) {
        try {
            // Utilizar el m閠odo toJSONCommand() para obtener el mensaje en formato JSON
            ...
            if (response != null && response.has("status") && response.getString("status").equals("error")) {
                // Si el servidor devuelve un estado de error, lanzar una excepci髇
                throw new communicationManagerException(response.getString("message"));
            }
